...
def make_dummy_openai(monkeypatch):
    ...
    mod.Model = types.SimpleNamespace(list=lambda: types.SimpleNamespace(data=[{"id": "gpt-4o-mini"}]))
    mod.ChatCompletion = types.SimpleNamespace(create=lambda **kw: types.SimpleNamespace(choices=[types.SimpleNamespace(message=types.SimpleNamespace(content='ok'))]))
