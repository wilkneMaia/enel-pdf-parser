...
def render_financial_flow(df_fin_view, total_custo):
    ...
    df_fat['Cor'] = df_fat['Valor (R$)'].apply(lambda x: '#EF553B' if x > 0 else '#00CC96') # Vermelho vs Verde
    ...
    with col_balanco:
        ...
        fig_bal = px.bar(
            ...
            y='Valor',
            color='Categoria',
            color_discrete_map={'Total Despesas': '#EF553B', 'Total Economia': '#00CC96'},
            text_auto='.2f'
        )
    ...
    with col_ranking:
        ...
        df_fat = df_fat.sort_values('Valor_Abs', ascending=True) # Crescente para plotar de cima pra baixo
    ...
    saldo_cobertura = (total_economia / total_despesas * 100) if total_despesas > 0 else 0
    if total_economia > 0:
        st.info(f"💡 A sua economia (Injeção/Descontos) abateu **{saldo_cobertura:.1f}%** das despesas brutas.")
