...
def render_taxometer(df_fin_view, total_custo):
    ...
    val_outros_imp = df_analise[df_analise["Categoria"] == "💸 Impostos (Fed/Est)"][
        "Valor (R$)"
    ].sum()

    total_impostos_fed_est = val_icms + val_pis + val_outros_imp
    total_ilum = df_analise[df_analise["Categoria"] == "🔦 Iluminação Pública"][
        "Valor (R$)"
    ].sum()
    ...
    if not df_treemap.empty:
        df_treemap = (
            df_treemap.groupby(["Item", "Categoria Macro", "Cor"])["Valor (R$)"]
            .sum()
            .reset_index()
        )
    ...
    with col_total:
        st.metric("🧾 Total Pago", f"R$ {total_custo:,.2f}")
    with col_minus:
        st.markdown(
            "<h2 style='text-align: center; color: gray;'>-</h2>",
            unsafe_allow_html=True,
        )
    with col_tax:
        st.metric(
            "🏛️ Mordida Fiscal",
            f"R$ {mordida_fiscal_total:,.2f}",
            delta=f"-{perc_mordida:.1f}%",
            delta_color="inverse",
        )
    with col_equal:
        st.markdown(
    ...
    with col_real:
        st.metric("⚡ Energia Real", f"R$ {valor_sem_imposto:,.2f}", delta="Produto")
    ...
    with c2:
        ...
        df_ranking = df_treemap[
            df_treemap["Categoria Macro"] != "⚡ Produto (Energia)"
        ].copy()
