...
def render_public_lighting(df_fin_view, df_med_view):
    ...
    if mask_ilum.any():
        ...
        if not df_cip.empty and not df_cons.empty:
            ...
            st.warning("Não foi possível cruzar os dados de Fatura com Medição para as mesmas datas.")
    else:
        st.info("Não foram identificados itens de Iluminação Pública (CIP/COSIP) nas faturas deste cliente.")
