...
st.set_page_config(
    page_title="Monitor de Execução (Logs)", page_icon="🔍", layout="wide"
)

# --- CONSTANTES ---
LOG_FILE = "logs/historico_geral.jsonl"
LOG_FILE = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "logs", "historico_geral.jsonl"
)
...
if df.empty:
    st.warning(
        "⚠️ Nenhum log encontrado. Execute o `main.py` primeiro para gerar dados."
    )
    st.stop()
...
sucessos = (
    len(df_view[df_view["status"] == "sucesso"]) if "status" in df_view.columns else 0
)
erros = (
    len(df_view[df_view["status"].str.contains("erro", case=False, na=False)])
...
with col1:
    st.metric("📄 Arquivos Processados", total_execucoes)
with col2:
...
with col3:
    st.metric(
        "❌ Erros",
        erros,
        delta=f"-{erros}" if erros > 0 else "0",
        delta_color="inverse",
    )
with col4:
    st.metric("💰 Valor Processado", f"R$ {total_money:,.2f}")
...
with c1:
    ...
    if "timestamp" in df_view.columns and not df_view.empty:
        ...
        timeline = (
            df_view.groupby(["hora", "status"]).size().reset_index(name="contagem")
        )
