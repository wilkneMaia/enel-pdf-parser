...
if len(meses_na_visao) >= 2:
    ...
    custo_anterior = df_sorted[df_sorted["Referência"] == mes_anterior][
        "Valor (R$)"
    ].sum()
...
# Renderiza KPIs
with col1:
    st.metric("💰 Custo Total (Período)", f"R$ {total_custo:,.2f}")
with col2:
    st.metric("⚡ Consumo Total", f"{consumo_real:,.0f} kWh")
